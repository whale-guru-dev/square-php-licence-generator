    ...
    public function getInfo(Request $request)
    {
        ...
        if (!empty($email) && !empty($password) && !empty($mac)) {
            ...
            if(!$user) {
                ...
                if(!$licence) {
                    return response()->json(['auth' => false, 'mac' => true, 'msg' => 0]);
                } else {
                    if ((strtotime($licence->expired) - time()) < 0) {
                        return response()->json(['auth' => true, 'mac' => true, 'plan' => $licence->plan->name,'msg' => 0]);
                    } else {
                        $now = time();
...
//                    $datediff = floor($expired - $now)/60/60/24;
                        ...
                        return response()->json(['auth' => true, 'mac' => true, 'plan' => $licence->plan->name,'msg' => $datediff]);
                    }
                }
            }
        } else {
            return response()->json(['auth' => false, 'mac' => false, 'msg' => 'Incorrect Request Format.']);
        }
    }
}
